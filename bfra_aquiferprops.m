function [k,Q0,D] = bfra_aquiferprops(q,dqdt,alate,blate,phi,A,D,L,soln,varargin)
%bfra_aquiferprops
...
bearly = 3;
aearly = bfra_pointcloudintercept(q,dqdt,bearly,'envelope','refqtls',earlyqtls);

% late-time intercept either Boussinesq 1904 or Rupp and Selker, 2005
switch soln
   case 'BS04'
      blate = 3/2;
      alate = bfra_pointcloudintercept(q,dqdt,blate,'envelope','refqtls',lateqtls,'mask',mask);
   case 'RS05'
      alate = bfra_pointcloudintercept(q,dqdt,blate,'envelope','refqtls',lateqtls,'mask',mask);
end
