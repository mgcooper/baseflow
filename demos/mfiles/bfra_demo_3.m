...
% When estimating aquifer properties using baseflow recession analysis, keep 
% in mind that the procedure involves two equations with known parameters. Therefore, 
% we can estimate two unknowns. The two equations are the early-time solution 
...
% $\phi$, and the "critical baseflow" $Q_0$. The critical baseflow marks the transition 
% from early-time to late-time discharge. At that point, the early-time and late-time 
% solutions are equivalent, which provides a method to solve for $Q_0$, leaving 
% three remaining unknowns.
% 
% Once the early-time and late-time _a_ and _b_ values are known, there are 
% two options for estimating the unknown aquifer properties provided by the baseflow 
% toolbox. 
% 
...
% other baseflow recession analysis studies. If this value is used in the |aquiferprops| 
