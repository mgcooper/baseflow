function [Y,Nsum] = nanmovmean(X,F,DIM)
   ...
   %  dimension DIM, while ignoring NaN values. It returns the array Y of
   %  averaged values and Nsum, an array indicating the number of non-NaN
   %  elements included in each average calculation. If F or DIM are not
   %  provided, they default to 1 and the first non-singleton dimension of X,
   %  respectively. The output arrays Y and Nsum have the same size as X.
   %
   % Inputs
   %  X - Input array, must be a 1D or 2D array.
   %  F - Width of moving average window, must be a positive integer.
   %  DIM - Dimension along which the moving average is computed.
   %
   % Outputs
   %  Y - Array of the same size as X, containing the computed moving averages.
   %  Nsum - Array of the same size as X, indicating the number of non-NaN
   %  elements used in calculating each average value.
   %
   % Example
