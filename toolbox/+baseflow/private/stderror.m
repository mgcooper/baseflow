function [SE,CI,PM,mu,sig] = stderror(data,varargin)
   ...
   %
   % [SE,CI,PM,mu,sig] = stderror(data,varargin) computes the compute the
   % standard error of the mean from the standard deviation of the data and the
   % sample size, and applies a critical t-value to compute 95% confidence
   % intervals.
   %
   ...
   % assume data is oriented columnwise, and we want the mean +/- stderr of each
   % row
...
function [data, dim, alpha] = parseinputs(data, funcname, varargin)
   parser = inputParser;
   parser.FunctionName = funcname;
   parser.addRequired('data', @isnumeric);
   parser.addParameter('alpha', 0.05, @isnumeric);
   parser.addParameter('dim', 2, @isnumeric);
