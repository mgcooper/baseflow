function [tf, istart, iend] = isminlength(tf, nmin)
   ...
   %  of the input column vector tf and returns true where the run length
   %  is nmin or more. istart and iend are the start and end indices of
   %  each stretch where the output tf is true. Adjacent runs that both
   %  satisfy nmin join into one stretch.
   %
