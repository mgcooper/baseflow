function updatecontents(folder)
   ...
   assert(ischar(folder), 'iosr:updateContents:invalidPath', ...
      '''directory'' should be a charater array (string)')
   assert(exist(folder,'dir')==7, 'iosr:updateContents:invalidPath', ...
   ...
   % get subfolders
   dirs = getcontents( ...
      folder,'filter','folders','rec',true,'path','full','sort',true);
   dirs = [{folder}; dirs];
   ...
   for d = 1:length(dirs)
      ...
      if ~isempty(temp)
         ...
         for f = 1:length(temp)
            ...
            % add H1 lines
            H1_lines = [H1_lines;
               {get_H1_line([dirs{d} filesep temp{f}])}]; %#ok<AGROW>
   ...
   % lines
   longest_word = max(cellfun(@length, ...
      files(cellfun(@(x) ~isempty(x),H1_lines))));
   ...
   for row=1:nrows
      if isempty(H1_lines{row})
         ...
         fprintf(fid, '%s\n', ...
            ['%   ' rowfilename ...
            repmat(' ', 1, longest_word-length(rowfilename)) ...
            ' - ' H1_lines{row,:}] ...
            );
      end
   end
   fprintf(fid, ...
      '%%    \n%%   %s on %s at %s.\n', ...
      'This file was generated by updatecontents.m', ...
      datestr(datetime('now'),'dd mmm yyyy'), ...
