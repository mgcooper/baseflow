function [Calm,Meta] = loadcalm(basinname,varargin)
   %LOADCALM Load CALM ALT data for a basin in the Bounds struct.
   %
   % Syntax
   %
   ...
   % NOTE: does not support multiple basins, use loadcalm("All_Basins") and
   % select since I think in terms of basins right now, not calm sites, this
   % accepts the basin name not the calm site name

   % PARSE INPUTS
   [basinname, version, t1, t2, aggfunc, minlength, mincoverage, ...
   ...
   [Calm, Meta] = aggregateCalm(Calm, Meta, aggfunc, minlength, ...
...
%% local functions
function [Calm, Meta] = aggregateCalm(Calm, Meta, aggfunc, minlength, ...
      mincoverage, minoverlap, maxdiff)

   % outside of this function, we can decide to fit trends to all sites that
   % pass the filters and then average those trends versus fitting to the
   ...
   switch aggfunc
      ...
      case 'robust'
         ...
         elseif sum(keep)==1
            ...
            Calm = Calm(:,keep);
            Meta = Meta(keep,:);
            Calm = aggregateCalm(Calm, Meta, 'avg', minlength, mincoverage, ...
               minoverlap, maxdiff);
            return
         end
...
function [basinname, version, t1, t2, aggfunc, minlength, mincoverage, ...
   ...
   parser.addParameter('aggfunc', 'none', @ischarlike);
   parser.addParameter('minlength', 10, @isnumericscalar);
   parser.addParameter('mincoverage', 0.8, @isnumericscalar);
   parser.addParameter('minoverlap', 0.5, @isnumericscalar);
   parser.addParameter('maxdiff', 0.5, @isnumericscalar);
