function h = mapbasins(Basins,varargin)
   ...
   h.map = axesm('MapProjection', proj, 'MapLatLimit', latlims, ...
      'MapLonLimit', lonlims);
      ...
      if usegeoshow == true
         ...
         for n = 1:numel(Basins)
            ...
            if isnan(cvar(n)) || cvar(n) == 0
               patchm(latn, lonn, 'FaceVertexCData', [0.255 0.255 0.255], ...
                  'FaceColor', 'flat', 'FaceAlpha', 0);
            else
               patchm(latn, lonn, 'FaceVertexCData', cvar(n), ...
                  'FaceColor', 'flat', 'FaceAlpha', facealpha);
            end
         end
         ax = gca;
