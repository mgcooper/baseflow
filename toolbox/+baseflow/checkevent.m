function h = checkevent(T,Q,q,dqdt,r,alltags,eventtag,varargin)
   ...
   % NOTE: two fits are shown in the legend b/c the first one is from the
   % provided inputs and the second one is computed in this function as a check
   % on the input
   ...
   % --------------- prep for fitting
   [eventT, eventQ, eventq, eventdqdt, eventr, annualT, annualQ, annualr] = ...
      prepinput(rowcol, alltags, eventtag, T, Q, q, dqdt, r);

   [tfit, qfit, dqfit, Qtstr, aQbstr, qfit0, dqfit0, Qtstr0, aQbstr0, rsq0] = ...
      prepfits(eventq, eventdqdt, eventT, fixb, order);
   ...
   title([datestr(eventT(1)) ' - ' datestr(eventT(end)) ...
      ' (Event ' num2str(eventtag) ')']);
