function [alpha,xmin,L,D,n,dat]=plfit(x, varargin)
   ...
   % ~~~~~~~~~~~~~~~~~~
   % mgc added D (K-S distance) to output
   % mgc added n (ntail) to output
   % mgc replaced legacy random number generator
   % ~~~~~~~~~~~~~~~~~~
   ...
   switch f_dattype
      ...
      case 'REAL'
         ...
         for xm=1:length(xmins)
            ...
            a    = n ./ sum( log(z./xmin) );
            % mgc a is alpha-1 i.e. the CDF exponent where cdf=(xmin/x)^a
            if nosmall
               if (a-1)/sqrt(n) > 0.1
                  dat(xm:end) = [];
                  xm = length(xmins)+1;
         ...
         if plotdiag == true
            ...
            %baseflow.deps.hline(dat(find(xmins>=12,1,'first')))
            baseflow.deps.hline(dat(find(xmins>=xmin,1,'first')))
      ...
      case 'INTS'
         ...
         L     = -alpha*sum(log(z))-n*log(zvec(find(vec<=alpha,1,'last'))- ...
            sum((1:xmin-1).^-alpha));
