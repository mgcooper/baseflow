function stats = bootstrapci(x,y,ab0,Fcost,Nboot,alpha,opts)
   ...
   %
   %  stats = bootstrapci(x,y,ab0,Fcost,Nboot,alpha,opts) computes standard
   %  errors, confidence levels, and mean values for parameters a and b of
   %  two-parameter model y = F(a,b) using initial parameter guess ab0, cost
   %  function Fcost, number of bootstrap resamples Nboot, and confidence level
   %  alpha.
   %
   % See also quantreg
